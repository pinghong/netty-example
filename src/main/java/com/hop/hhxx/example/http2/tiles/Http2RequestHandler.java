...
package com.hop.hhxx.example.http2.tiles;
...
import io.netty.example.http2.tiles.*;
...
public class Http2RequestHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private void handlePage(ChannelHandlerContext ctx, String streamId, int latency, FullHttpRequest request) {
        byte[] body = io.netty.example.http2.tiles.Html.body(latency);
        ByteBuf content = ctx.alloc().buffer(io.netty.example.http2.tiles.Html.HEADER.length + body.length + io.netty.example.http2.tiles.Html.FOOTER.length);
        content.writeBytes(io.netty.example.http2.tiles.Html.HEADER);
